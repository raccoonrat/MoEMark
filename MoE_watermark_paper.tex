...
这种方法的核心思想是将专家激活的\textit{序列} (sequence) 本身作为水印信号~\cite{ref1}。水印不再是静态的权重扰动，而是一种动态的、基于输入的\textit{行为}。从信息论的角度来看，路由状态空间提供了多个信息维度用于编码水印，包括激活模式、排列顺序和权重量化~\cite{ref9}。~\cite{ref1}中描述了两种概念性方法：
...
\item \textbf{有效性:} 这种方法并非绝对可靠，但其有效性依赖于冗余设计。相关研究证据表明，在门控网络中结合使用这种策略和"冗余" (redundancy) 设计（例如，可能将信号分散到多个路由层或决策点），可以实现"50-70\% 的信号恢复" (50-70\% signal recovery)~\cite{ref1}。从信息论的角度，这种冗余设计可以通过增加编码空间的维度来提高鲁棒性下界~\cite{ref9}。
\end{itemize}
...
\item \textbf{防御行为:} 当路由器识别出输入可能受到了对抗性攻击（例如，旨在干扰其决策的噪声或扰动）时，它会"自适应地将每个受干扰的样本路由到相应的鲁棒专家" (adaptively routes each perturbed sample to the corresponding expert that exhibits robustness against that perturbation)~\cite{ref1}。简而言之，路由器学会了在检测到攻击时"激活仅有鲁棒的专家" (activate only the robust experts)，从而绕过攻击的影响~\cite{ref1}。这种机制在理论上可以通过最大化条件互信息$I(\mathbf{m}; \mathcal{S}'(x) | \mathcal{A})$来形式化，其中$\mathcal{A}$表示攻击变换~\cite{ref9}。
\end{itemize}
...
\begin{enumerate}
\item \textbf{可扩展的检测工具:} 最大的开放性问题是，如何在无法访问激活日志的情况下（例如，通过公共 API）可靠地验证基于路由的水印~\cite{ref1}。这对于黑盒溯源至关重要。从信息论的角度，这涉及到在假设检验框架下最大化检测功率（Detection Power），同时控制假阳性率~\cite{ref9}。
\item \textbf{标准化基准:} 目前缺乏专门针对 MoE 漏洞（如路由干扰、剪枝、蒸馏）的水印评测基准。开发类似于 WMD 或 AIGCDetect 但专注于 MoE 的标准将是必要的~\cite{ref1}。这些基准应该考虑信息容量、鲁棒性和检测性能之间的权衡，并基于形式化的信息论框架~\cite{ref9}。
...
\bibitem{ref9}
MoE路由水印的信息论形式化定义 (Information-Theoretic Formalization of MoE Routing Watermarking), moe\_routing\_watermark.md, 2025.
