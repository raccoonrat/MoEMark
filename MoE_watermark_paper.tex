...
\begin{itemize}
\item \textbf{机制:} 攻击者可以利用路由器（Gating Network）通常轻量级的特性，对其进行重新训练、微调或注入噪声~\cite{ref1}。
\item \textbf{影响:} 这种攻击\textit{直接}针对基于行为的水印，旨在"消除控制平面信号"~\cite{ref1}。任何依赖于特定路由逻辑（如 BadSwitch 改编版）的水印都可能因此失效。
\end{itemize}
